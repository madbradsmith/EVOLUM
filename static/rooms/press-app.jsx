/* =====================================================================
   EVOLUM Press / Outreach — 3-zone dock
   Sections (left) | Detail (main) | Preview (right)
   ===================================================================== */
const PressApp = () => {
  const project = PROJECTS.find(p => p.id === 'p01');
  const [active, setActive] = React.useState('social');
  const [plat, setPlat] = React.useState('linkedin');
  const [angle, setAngle] = React.useState('cherry');
  ...
  const generate = () => {
    ...
    setPost('');
    setTimeout(() => {
      const base = SAMPLE_POSTS[plat]?.[angle] || `[${plat} \u00b7 ${angle}]\n\nA fresh take coming through Hitchcock\u2019s pipeline. ${mention ? '\n\nNote: ' + mention : ''}`;
      setPost(base + (mention ? `\n\n\u2014 ${mention}` : ''));
      setGenerating(false);
    }, 1400);
  };
  ...
  const config = {
    project,
    roomName: 'Press · Outreach',
    panels: {
      sections: { meta: { title: 'Outreach',   icon: 'send',     sub: 'this project' } },
      detail:   { meta: { title: PRESS_SECTIONS.find(s=>s.id===active)?.name || 'Detail', icon: 'sparkle', sub: '' } },
      preview:  { meta: { title: active==='public' ? 'Live Page Preview' : 'Post Preview', icon: 'undock', sub: active==='public' ? 'evolumstudio.com/film/...' : (post ? `${post.length} chars` : 'empty') } },
    },
    renderPanel: (id) => {
      switch (id) {
        case 'sections': return <PressSectionsPanel active={active} setActive={setActive}/>;
        case 'detail':   return <PressDetail active={active} plat={plat} setPlat={setPlat} angle={angle} setAngle={setAngle} mention={mention} setMention={setMention} post={post} generate={generate} generating={generating}/>;
        case 'preview':  return <PressPreview active={active} plat={plat} angle={angle} post={post} generating={generating} mention={mention}/>;
        default: return null;
      }
    },
    statusItems: [
      { lbl:'PLATFORM', num: PLATFORMS.find(p=>p.id===plat)?.name?.toUpperCase() || '—' },
      { lbl:'ANGLE',    num: ANGLES.find(a=>a.id===angle)?.name?.toUpperCase() || '—' },
      { lbl:'QUEUED',   num: BUFFER_QUEUE.filter(b=>b.state!=='posted').length },
      { lbl:'POSTED',   num: BUFFER_QUEUE.filter(b=>b.state==='posted').length },
    ],
    statusRight: (
      <React.Fragment>
        <span><span style={{display:'inline-block',width:6,height:6,borderRadius:'50%',background:'var(--ok)',marginRight:5,verticalAlign:'middle',boxShadow:'0 0 6px var(--ok-glow)'}}/>Buffer + Stripe connected</span>
  ...
  return <PressRoom config={config}/>;
};

const PressRoom = ({ config }) => {
  ...
  const [rightW, setRightW] = React.useState(420);
  ...
  const splitR = (e) => {
    ...
    const onMove = (ev) => setRightW(Math.max(320, Math.min(640, startW - (ev.clientX - start))));
    const onUp = () => { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp); };
    window.addEventListener('mousemove', onMove); window.addEventListener('mouseup', onUp);
  };
  ...
  return (
    <React.Fragment>
      ...
      <div className="menubar">
        <button>File</button><button>Edit</button><button>View</button><button>Generate</button><button>Schedule</button><button>Window</button><button>Help</button>
        <span className="mb-spacer"/>
        <button className="btn primary" style={{padding:'3px 10px', marginRight:6}}><Icon name="sparkle" size={11}/> Generate Batch (10)</button>
        <span className="layout-name">LAYOUT · PRESS</span>
...
ReactDOM.createRoot(document.getElementById('app')).render(<PressApp/>);
