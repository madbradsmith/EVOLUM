...
def _select_user_image(current_files: list[Path], lookup_key: str, last_used_name: str = "") -> Optional[Path]:
    ...
    candidates = sorted(current_files, key=lambda p: p.name.lower())
    ...
    for offset in range(len(candidates)):
        ...
        if len(candidates) == 1 or candidate.name != last_used_name:
            _user_rotation_counters[lookup_key] = (start_idx + offset + 1) % len(candidates)
            return candidate

    candidate = candidates[start_idx % len(candidates)]
    _user_rotation_counters[lookup_key] = (start_idx + 1) % len(candidates)
    return candidate
...
def _select_stock_image(stock_files: list[Path], lookup_key: str, last_used_name: str = "") -> Optional[Path]:
    ...
    for offset in range(len(candidates)):
        candidate = candidates[(start_idx + offset) % len(candidates)]
        if len(candidates) == 1 or candidate.name != last_used_name:
            _stock_rotation_counters[key] = (start_idx + offset + 1) % len(candidates)
            return candidate

    candidate = candidates[start_idx % len(candidates)]
    _stock_rotation_counters[key] = (start_idx + 1) % len(candidates)
    return candidate
